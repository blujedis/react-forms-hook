...
How to test, build export Storybook and generating new Components.
...
I've included a handy NodeJS util file under `tools` called `generate.js`. Instead of copy pasting components to create a new component, you can instead run this command to generate all the files you need to start building out a new component. To use it:

```
yarn gen ComponentName
yarn gen BlueprintName ComponentName
yarn gen ComponentName --force (or -f to overwrite) 
```
...
The default templates for each file can be modified under `tools/blueprints`.

### Installing Component Library Locally

Let's say you have another project (`myapp`) on your machine that you want to try installing the component library into without having to first publish the component library. In the `myapp` directory, you can run:

```
yarn add ../react-component-library
```

which will install the local component library as a dependency in `myapp`. It'll then appear as a dependency in `package.json` like:
...
The `"prepublishOnly": "yarn build"` script in `package.json` will execute before publish occurs, ensuring the `build/` directory and the compiled component library exist.
...
```
yarn add git+https://github.com/USERNAME/react-component-library.git#branch-name
```

OR

```
yarn add github:USERNAME/react-component-library#branch-name
```
...
For example, let's say you installed `my-component-library` into your project. To use the exported variables/mixins, in a SASS file you would do the following:

```Sass
@import '~my-component-library/build/typography';
...
- For Stylus, install stylus: `yarn add stylus -D`
- For Less, install less: `yarn add less -D`
...
If you want to use [`styled-components`](https://styled-components.com/), the changes required are a bit more involved. As such, I've created a branch where I've got `styled-components` working in this component library, [check it out here](https://github.com/USERNAME/react-component-library/tree/styled-components).
...
```
import TestComponent from 'my-component-library/build/TestComponent';
```
